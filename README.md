...
- `officegen-v5.2.0-x86_64-unknown-linux-gnu.tar.gz`
- `officegen-v5.2.0-aarch64-unknown-linux-gnu.tar.gz`
- `officegen-v5.2.0-x86_64-apple-darwin.tar.gz`
- `officegen-v5.2.0-aarch64-apple-darwin.tar.gz`
- `officegen-v5.2.0-x86_64-pc-windows-msvc.zip`
- `officegen-v5.2.0-aarch64-pc-windows-msvc.zip`
...
Officegen v5.2.0 is the Rust-native Office/PDF automation release. It keeps the v4.5 safety contract, removes Node from the runtime path, and adds practical authoring/editing flows for PPTX, DOCX, XLSX, PDF, templates, workflow manifests, semantic diff, and scoped previews. It is still intentionally not a full Office clone; unsupported surfaces fail closed instead of pretending to work.
...
cargo build --release --locked
# macOS/Linux
node scripts/native-release-smoke.mjs --bin target/release/officegen --expected-version 5.2.0
npm run v5:acceptance -- --bin target/release/officegen --expected-version 5.2.0
# Windows
node scripts/native-release-smoke.mjs --bin target/release/officegen.exe --expected-version 5.2.0
...
```bash
npm run native:assets:check -- --version 5.2.0 --dist-dir dist --include-installers
```
