...
export function localText(xml, localName) {
...
export function exactText(xml, tagName) {
...
export function replaceAllXmlText(input, from, to) {
...
export function replaceFirstBlock(xml, pattern, predicate, replacer) {
    let ordinal = 0;
    let matchCount = 0;
    let replaced = false;
    const next = xml.replace(pattern, (match) => {
        ordinal += 1;
        if (!predicate(match, ordinal))
            return match;
        matchCount += 1;
        if (replaced)
            return match;
...
export function setFirstTextInBlock(block, tagName, text) {
    const tag = escapeRegExp(tagName);
    const pattern = new RegExp(`<${tag}(?=\\s|>)([^>]*)>[\\s\\S]*?<\\/${tag}>`, "g");
    let replaced = false;
    let sawText = false;
    const next = block.replace(pattern, (_match, attrs) => {
        sawText = true;
        if (!replaced) {
            replaced = true;
...
export function preview(text, limit = 120) {
    if (!text)
        return undefined;
