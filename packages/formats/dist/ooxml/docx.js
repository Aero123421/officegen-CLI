...
export const DOCX_STORY_GRAPH_VERSION = "officegen.docx.storyGraph@0.1";
export const DOCX_RUN_GRAPH_VERSION = "officegen.docx.runGraph@0.1";
...
function inspectParagraphGraph(xml, context) {
    const runGraph = emptyRunGraph();
    ...
    for (const [runIndex, runMatch] of [...xml.matchAll(/<w:r\b[\s\S]*?<\/w:r>/g)].entries()) {
        ...
        if (previousRunId)
            addRunEdge(runGraph, previousRunId, runId, "next");
        previousRunId = runId;
...
function inspectParagraphStructuralNodes(xml, context, paragraphId, markers) {
    const graph = emptyRunGraph();
    const matches = [];
    const add = (type, key, index, attrs = {}, text) => {
    ...
    for (const item of matches) {
        ...
        if (previousId)
            addRunEdge(graph, previousId, item.node.nodeId, "next");
        previousId = item.node.nodeId;
    }
    return graph;
}
function addRunNode(graph, node) {
    graph.nodes.push(node);
}
function addRunEdge(graph, from, to, relation) {
    graph.edges.push({ from, to, relation, untrusted: true });
}
function appendRunGraph(target, source) {
...
function summarizeStories(graph) {
    return {
...
function summarizeRunGraph(graph) {
...
function contentControlProps(xml) {
