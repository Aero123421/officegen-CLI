...
function parseSimpleMermaid(source) {
    const edges = [];
    const nodes = new Set();
    for (const rawLine of source.split(/\r?\n/)) {
        const line = rawLine.trim();
        if (!line || /^(graph|flowchart|sequenceDiagram|classDiagram)\b/.test(line))
            continue;
...
function buildDiagramSvg(graph, width, height, title) {
    const top = title ? 64 : 32;
    const gap = (width - 96) / Math.max(1, graph.nodes.length - 1);
    const positions = new Map(graph.nodes.map((node, index) => [
        node,
        {
            x: graph.nodes.length === 1 ? width / 2 : 48 + index * gap,
            y: top + (index % 2) * 120
        }
    ]));
    ...
    const nodes = graph.nodes
        ...
        return `<g><rect x="${pos.x - 64}" y="${pos.y}" width="128" height="56" rx="6" fill="#f6f8fa" stroke="#2f6f73" stroke-width="2"/><text x="${pos.x}" y="${pos.y + 34}" text-anchor="middle" font-family="Arial, sans-serif" font-size="14" fill="#111">${escapeXml(node)}</text></g>`;
    })
        .join("");
