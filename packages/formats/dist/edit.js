...
import { SELECTOR_GRAPH_LOW_CONFIDENCE_THRESHOLD, objectGraphHash as hashObjectGraph, selectionLockForNode, selectorResolutionNextActions, sourceFingerprintForNode } from "./graphs/selectorGraph.js";
...
async function applyOfficeOperation(context, format, operation, objectMap, index) {
    ...
    if (format === "pptx" && op === "pptx.formatAllTitles") {
        return editPptxFormatAllTitles(zip, operation, objectMap);
    }
    if (format === "pptx" && op === "pptx.replaceBodyBullets") {
        return editPptxReplaceBodyBullets(zip, operation, objectMap);
    }
    if (format === "pptx" && op === "pptx.fitContentToPlaceholder") {
        return editPptxFitContent(zip, operation, objectMap);
    }
    if (format === "pptx" && op === "pptx.alignObjects") {
        return editPptxAlignObjects(zip, operation, objectMap);
    }
    if (format === "pptx" && op === "pptx.distributeObjects") {
        return editPptxDistributeObjects(zip, operation, objectMap);
    }
    if (format === "pptx" && op === "pptx.setAltText") {
        return editPptxSetAltText(zip, operation, objectMap);
    }
    if (format === "pptx" && op === "pptx.setSpeakerNotes") {
        return editPptxSetSpeakerNotes(zip, operation);
    }
    ...
    if (format === "docx" && op === "docx.replaceTextSmart") {
        const smart = operation;
        return smart.selector ? editDocxReplaceTextSmartSelected(zip, smart, objectMap) : replaceTextInEditableParts(zip, format, smart.from, smart.to);
    }
    if (format === "docx" && op === "docx.setTableCellText") {
        return editDocxSetTableCellText(zip, operation, objectMap);
    }
    ...
    if (format === "xlsx" && op === "xlsx.definedName.delete") {
        return editXlsxDefinedName(zip, operation, "delete");
    }
...
async function editPptxReplaceBodyBullets(zip, operation, objectMap) {
    ...
    if (!target?.sourcePath)
        throw new Error(`SELECTOR_NOT_FOUND: no body shape found on slide ${operation.slide}.`);
    const xml = (await readZipText(zip, target.sourcePath)) ?? "";
    const next = updateSelectedPptxShape(xml, target, (shape) => replaceTextBodyParagraphs(shape, operation.items, operation.spaceBeforeForLevel1ExceptFirst));
    if (next !== xml)
        zip.file(target.sourcePath, next);
...
async function editPptxFitContent(zip, operation, objectMap) {
    const target = singleMatch(objectMap, operation.selector);
    if (!target.sourcePath || target.kind !== "shape")
        throw new Error("SELECTOR_NOT_FOUND: selected object is not a PPTX text shape.");
    const xml = (await readZipText(zip, target.sourcePath)) ?? "";
...
function ensurePptxNormAutofit(shapeXml, minFontSize) {
    ...
    if (/<a:bodyPr\b[^>]*\/>/.test(cleaned)) {
        return cleaned.replace(/<a:bodyPr\b([^>]*)\/>/, (_match, attrs) => `<a:bodyPr${attrs}>${autofit}</a:bodyPr>`);
    }
    return cleaned.replace(/(<a:bodyPr\b[^>]*>)([\s\S]*?)(<\/a:bodyPr>)/, (_match, open, body, close) => `${open}${body}${autofit}${close}`);
}
async function editPptxAlignObjects(zip, operation, objectMap) {
    const targets = operation.selectors.map((selector) => singleMatch(objectMap, selector)).filter((entry) => entry.bounds);
    if (targets.length < 2)
        throw new Error("SELECTOR_NOT_FOUND: pptx.alignObjects requires at least two bounded objects.");
    ...
    for (const target of targets.slice(1)) {
        ...
        if (operation.mode === "left")
            nextBounds.x = anchor.x;
        if (operation.mode === "right")
            nextBounds.x = anchor.x + anchor.width - bounds.width;
        if (operation.mode === "center")
            nextBounds.x = anchor.x + (anchor.width - bounds.width) / 2;
        if (operation.mode === "top")
            nextBounds.y = anchor.y;
        if (operation.mode === "bottom")
            nextBounds.y = anchor.y + anchor.height - bounds.height;
        if (operation.mode === "middle")
            nextBounds.y = anchor.y + (anchor.height - bounds.height) / 2;
        changed = (await setPptxTargetBounds(zip, target, nextBounds)) || changed;
    }
    return changed;
}
async function editPptxDistributeObjects(zip, operation, objectMap) {
    const targets = operation.selectors.map((selector) => singleMatch(objectMap, selector)).filter((entry) => entry.bounds)
        .sort((left, right) => operation.axis === "x" ? left.bounds.x - right.bounds.x : left.bounds.y - right.bounds.y);
    if (targets.length < 3)
        throw new Error("SELECTOR_NOT_FOUND: pptx.distributeObjects requires at least three bounded objects.");
    ...
    for (const [index, target] of targets.entries()) {
        ...
        const bounds = { ...target.bounds };
        if (operation.axis === "x")
            bounds.x = start + gap * index;
        else
            bounds.y = start + gap * index;
        changed = (await setPptxTargetBounds(zip, target, bounds)) || changed;
    }
    return changed;
}
async function editPptxSetAltText(zip, operation, objectMap) {
    const target = singleMatch(objectMap, operation.selector);
    if (!target.sourcePath || !["shape", "picture", "chart"].includes(target.kind))
        throw new Error("SELECTOR_NOT_FOUND: selected object cannot receive alt text.");
    const xml = (await readZipText(zip, target.sourcePath)) ?? "";
    const next = replacePptxObjectBlock(xml, target.kind, String(target.selectorHints?.shapeId ?? ""), Number(target.selectorHints?.shapeIndex ?? stableOrdinal(target.stableObjectId)), (block) => block.replace(/<p:cNvPr\b([^>]*)\/>/, (_match, attrs) => {
        let nextAttrs = attrs;
        if (operation.title !== undefined)
            nextAttrs = upsertXmlAttr(nextAttrs, "title", operation.title);
...
async function setPptxTargetBounds(zip, target, bounds) {
    if (!target.sourcePath || !["shape", "picture", "chart"].includes(target.kind)) {
    ...
    const ordinal = Number(target.kind === "picture"
...
async function editPptxSetSpeakerNotes(zip, operation) {
    const slidePaths = await getSlidePaths(zip);
    const slidePath = slidePaths[operation.slide - 1];
    if (!slidePath)
        throw new Error(`SELECTOR_NOT_FOUND: pptx slide ${operation.slide} not found.`);
...
async function editDocxReplaceTextSmartSelected(zip, operation, objectMap) {
    const target = singleMatch(objectMap, operation.selector);
    if (!target.sourcePath)
        throw new Error("SELECTOR_NOT_FOUND: selected DOCX object has no sourcePath.");
    const xml = (await readZipText(zip, target.sourcePath)) ?? "";
    const ordinal = Number(target.selectorHints?.paragraph ?? stableOrdinal(target.stableObjectId));
...
async function editDocxSetTableCellText(zip, operation, objectMap) {
    const target = singleMatch(objectMap, operation.selector);
    if (!target.sourcePath || target.kind !== "tableCell")
        throw new Error("SELECTOR_NOT_FOUND: selected object is not a DOCX table cell.");
    const xml = (await readZipText(zip, target.sourcePath)) ?? "";
...
function smartReplaceDocxParagraph(paragraph, from, to) {
    if (!from)
        return paragraph;
    ...
    let used = false;
    return paragraph.replace(/<w:t\b([^>]*)>[\s\S]*?<\/w:t>/g, (_match, attrs) => {
        if (used)
...
async function editXlsxDefinedName(zip, operation, mode) {
    ...
    const xml = (await readZipText(zip, path)) ?? "";
    if (!operation.name)
        throw new Error("SCHEMA_INVALID: defined name requires a non-empty name.");
    ...
    if (mode === "set") {
        ...
        if (!ref)
            throw new Error("SCHEMA_INVALID: xlsx.definedName.set requires ref.");
        const definedName = `<definedName name="${name}">${ref}</definedName>`;
        if (/<definedNames\b[\s\S]*?<\/definedNames>/.test(next))
...
function assertSafeXlsxFormula(formula) {
...
async function markXlsxRecalcNeeded(zip) {
...
function replacePptxObjectBlock(xml, kind, shapeId, ordinal, updater) {
    ...
    return xml.replace(pattern, (block) => {
        ...
        const candidateId = /\bid="([^"]+)"/.exec(cNvPr)?.[1];
        if (shapeId && candidateId !== shapeId)
            return block;
        if (!shapeId && Number.isFinite(ordinal) && index !== ordinal)
            return block;
        if (!shapeId && !Number.isFinite(ordinal) && index !== 1)
            return block;
...
function resolveCurrentPptxShape(xml, target) {
    ...
    if (byStableId)
        return { shapeId: byStableId.shapeId, shapeIndex: byStableId.shapeIndex };
    ...
    if (targetShapeId) {
        ...
        if (byShapeId)
            return { shapeId: byShapeId.shapeId, shapeIndex: byShapeId.shapeIndex };
    }
    ...
    if (Number.isFinite(targetShapeIndex)) {
        ...
        if (byShapeIndex)
            return { shapeId: byShapeIndex.shapeId, shapeIndex: byShapeIndex.shapeIndex };
    }
    return undefined;
}
function preferPptxShapeCandidate(candidates, target) {
    if (candidates.length <= 1)
        return candidates[0];
...
function validationFailures(selectorResult, minSelectorConfidence = 0) {
...
function stalePlanFailures(selectorResult, options, operations) {
    ...
    if (options.selectionLock) {
        ...
        if (lock.objectGraphHash !== selectorResult.objectGraphHash) {
            ...
            const currentLock = selectorResult.resolutions.find((resolution) => (!lock.nodeId || resolution.selectionLock?.nodeId === lock.nodeId) &&
                (!lock.sourceFingerprint || resolution.selectionLock?.sourceFingerprint === lock.sourceFingerprint))?.selectionLock;
            if (!currentLock) {
                failures.push({
                    operationIndex: -1,
                    op: "selectionLock",
...
async function buildPatchPlan(format, inputBytes, operations, opResults, selectorResult, transaction, store, initialPartPaths = new Set(), currentPartPaths = new Set()) {
...
async function patchPlanTouchedParts(transaction, store, initialPartPaths, currentPartPaths) {
    ...
    for (const entry of transaction.snapshot()) {
        ...
        if (entry.existed && current === undefined) {
            byPath.set(entry.path, {
                path: entry.path,
                change: "deleted",
                beforeSha256: sha256Part(entry.value),
                sourceFingerprint: partSourceFingerprint(entry.path, entry.value)
            });
            continue;
        }
        if (!entry.existed && current !== undefined) {
        ...
        if (entry.existed && current !== undefined) {
            const beforeSha256 = sha256Part(entry.value);
    ...
    for (const path of currentPartPaths) {
        if (initialPartPaths.has(path) || byPath.has(path))
            continue;
...
function partSourceFingerprint(path, value) {
...
function editAbortResult(format, skipped, opResults, operations, caveats, selectorResult, dryRun, inputBytes) {
    const errors = opResults.filter(isRequiredEditFailure);
...
function isRequiredEditFailure(result) {
...
function selectorStatus(matchCount, confidence) {
    ...
    if (matchCount > 1)
        return "ambiguous";
    if (confidence !== undefined && confidence < SELECTOR_GRAPH_LOW_CONFIDENCE_THRESHOLD)
        return "low_confidence";
    return "matched";
}
function selectorResolutionV2ForObjectMap(selector, matches, graphNodesByStableId, graph, currentObjectGraphHash, status, confidence) {
    const matchedNode = matches.length === 1 ? graphNodesByStableId.get(matches[0]?.stableObjectId ?? "") : undefined;
...
function resolveMatches(objectMap, selector, options = {}) {
    ...
    if (selector.sourcePath)
        candidates = candidates.filter((entry) => entry.sourcePath === selector.sourcePath || entry.selectorHints?.sourcePath === selector.sourcePath);
    if (selector.xmlPath)
        candidates = candidates.filter((entry) => entry.xmlPath === selector.xmlPath || entry.selectorHints?.xmlPath === selector.xmlPath);
    if (selector.page !== undefined)
        candidates = candidates.filter((entry) => Number(entry.selectorHints?.page ?? entry.selectorHints?.slide ?? entry.selectorHints?.sheet) === selector.page);
    if (selector.story)
        candidates = candidates.filter((entry) => entry.selectorHints?.story === selector.story || entry.selectorHints?.partKind === selector.story);
    if (selector.paragraph !== undefined)
        candidates = candidates.filter((entry) => Number(entry.selectorHints?.paragraph) === selector.paragraph);
    if (selector.table !== undefined)
        candidates = candidates.filter((entry) => Number(entry.selectorHints?.table ?? entry.selectorHints?.tableIndex) === selector.table);
    if (selector.row !== undefined)
        candidates = candidates.filter((entry) => Number(entry.selectorHints?.row) === selector.row);
    if (selector.column !== undefined)
        candidates = candidates.filter((entry) => Number(entry.selectorHints?.column) === selector.column);
    if (selector.range)
        candidates = candidates.filter((entry) => String(entry.selectorHints?.range ?? entry.selectorHints?.ref ?? "") === selector.range);
    if (selector.relationshipId)
        candidates = candidates.filter((entry) => entry.selectorHints?.relationshipId === selector.relationshipId || entry.media?.relationshipId === selector.relationshipId);
    if (selector.assetPath)
        candidates = candidates.filter((entry) => entry.selectorHints?.assetPath === selector.assetPath || entry.media?.assetPath === selector.assetPath);
    if (selector.commentId)
        candidates = candidates.filter((entry) => String(entry.selectorHints?.commentId ?? "") === selector.commentId);
    if (selector.revisionId)
        candidates = candidates.filter((entry) => String(entry.selectorHints?.revisionId ?? "") === selector.revisionId);
...
async function editPdf(input, operations, options, selectorResult) {
    const inputSha256 = selectorResult?.inputSha256 ?? sha256Bytes(input.bytes);
    const sourceFingerprint = inputSourceFingerprint(input.bytes);
