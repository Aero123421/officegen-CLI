...
export async function embedPdfFonts(pdf, textSamples) {
    ...
    {
        throw new OfficegenError("RENDER_FONT_UNSUPPORTED", "PDF text contains CJK or other Unicode characters, but no embeddable CJK font was found. Set OFFICEGEN_PDF_FONT to a .ttf/.otf font path or use native export.", { searched: cjkFontCandidates(), attempts, env: "OFFICEGEN_PDF_FONT", preview: text.slice(0, 120) });
    }
}
export function ensurePdfTextEncodable(value, font, context) {
    try {
        font.encodeText(value);
        return value;
    }
    catch {
        throw new OfficegenError("RENDER_FONT_UNSUPPORTED", "PDF text contains glyphs that cannot be encoded by the active PDF font.", { context, preview: value.slice(0, 80) });
    }
}
function needsUnicodeFont(value) {
    return /[^\u0009\u000a\u000d\u0020-\u007e]/.test(value);
}
function findCjkFontPaths() {
...
function bundledJapaneseFontCandidates() {
    try {
...
//# sourceMappingURL=pdfFonts.js.map