...
async function renderPptx(ir: DocumentIR, options: RenderOptions): Promise<RenderResult> {
  ...
  for (const [sectionIndex, section] of slides.entries()) {
    ...
    for (const block of blocks) {
      ...
      if (type === "list" || block.items?.length) {
        ...
        slide.addText(items.map((item: string) => ({ text: item, options: { bullet: { type: "ul" } } })), { x: 0.78, y, w: 11.6, h: Math.min(2.8, items.length * 0.28 + 0.15), fontSize: 13.5, color: palette.text, fit: "shrink" });
  ...
  if (diagnostics.length) caveats.push("Some PPTX blocks exceeded the simple layout budget; inspect/view/diagnose the output before using it as a final deck.");
...
function normalizedSections(ir: DocumentIR): Array<{ title?: string; body?: string | string[]; blocks?: BlockIR[]; rows?: Array<Record<string, unknown> | unknown[]> }> {
...
function blocksFromBody(body: string | string[] | undefined): BlockIR[] {
  const lines = toLines(body);
  return lines.length ? [{ type: "paragraph", text: lines.join("\n") }] : [];
}

function normalizeTableRows(rows: Array<Record<string, unknown> | unknown[]>): unknown[][] {
