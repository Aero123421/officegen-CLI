...
function toPages(inspected: InspectResult, options: ViewOptions): ViewPage[] {
  ...
  if (inspected.trusted.format === "pdf") {
    const pages = ((inspected.untrusted.pages as Array<Record<string, unknown>>) ?? []).slice(0, maxPages);
    return pages.map((page, index) => buildPdfPage(page, index + 1, format));
  }
  return [];
}

function buildSlidePage(slide: Record<string, unknown>, page: number, format: "svg" | "html"): ViewPage {
  const objects = ((slide.textObjects as ObjectMapEntry[]) ?? []).map((entry, index) => ({
    ...entry,
...
function buildSheetPage(sheet: Record<string, unknown>, page: number, format: "svg" | "html"): ViewPage {
