...
export function readPptxBlockTransformContent(block: string): string | undefined {
  const pXfrm = /<p:xfrm\b[^>]*>([\s\S]*?)<\/p:xfrm>/i.exec(block);
  if (pXfrm) return pXfrm[1];
  const aXfrm = /<a:xfrm\b[^>]*>([\s\S]*?)<\/a:xfrm>/i.exec(block);
  if (aXfrm) return aXfrm[1];
  return undefined;
}

function extractBoundsFromTransformContent(content: string): ObjectBounds | undefined {
...
export function applyBoundsToPptxBlock(block: string, bounds: ObjectBounds): string {
