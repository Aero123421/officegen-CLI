import {
  normalizeText,
  type ObjectGraph,
  type ObjectGraphBBox,
  type ObjectGraphEvidence,
...
export interface SelectorGraphSelector {
  ...
  rightOf?: string | { text: string; slide?: number; type?: string | string[] };
  nearestTo?: { slide?: number; x: number; y: number; maxDistance?: number };
}
...
interface ApplyResult {
  ...
  evidence: ObjectGraphEvidence[];
  forcedAmbiguity?: SelectorGraphAmbiguity;
}

export function resolveGraphSelector(graph: ObjectGraph, selector: SelectorGraphSelector): SelectorGraphResolution {
  ...
  const status: SelectorGraphStatus = matches.length === 0
    ? "not-found"
    : ambiguity.ambiguous
      ? "ambiguous"
      : lowConfidence
...
function applySelector(graph: ObjectGraph, initialCandidates: Candidate[], selector: SelectorGraphSelector): ApplyResult {
  ...
  if (selector.relation) {
    ...
    if (relationResult.forcedAmbiguity) return { candidates, evidence, forcedAmbiguity: relationResult.forcedAmbiguity };
  }
  ...
  if (selector.rightOf) {
    ...
    if (rightOfResult.forcedAmbiguity) return { candidates, evidence, forcedAmbiguity: rightOfResult.forcedAmbiguity };
  }
...
function applyRelation(graph: ObjectGraph, candidates: Candidate[], relation: SelectorGraphRelationSelector): ApplyResult {
  ...
  const related = candidates.filter(({ node }) => graph.edges.some((edge) => {
    if (edge.relation !== relation.relation) return false;
...
function applyRightOf(graph: ObjectGraph, candidates: Candidate[], selector: NonNullable<SelectorGraphSelector["rightOf"]>, fallbackSlide: number | undefined): ApplyResult {
  ...
  const anchors = graph.nodes.filter((node) => {
    if (!node.bbox || !nodeTextContains(node, spec.text)) return false;
    if (spec.slide !== undefined && node.source.slide !== spec.slide) return false;
  ...
  if (anchors.length === 0) return { candidates: [], evidence: [item] };
  if (anchors.length > 1) {
    return {
  ...
  const anchor = anchors[0] as ObjectGraphNode;
  ...
  const ranked = candidates
    ...
    .map((candidate) => ({ ...candidate, distance: horizontalGap(candidate.node.bbox as ObjectGraphBBox, anchor.bbox as ObjectGraphBBox) }))
    .sort((left, right) => (left.distance ?? 0) - (right.distance ?? 0));
  const best = ranked[0];
  if (!best) return { candidates: [], evidence: [item] };
...
function applyNearestTo(candidates: Candidate[], point: NonNullable<SelectorGraphSelector["nearestTo"]>): ApplyResult {
...
function relationAnchor(graph: ObjectGraph, relation: SelectorGraphRelationSelector): {
  ids?: Set<string>;
  candidates?: Candidate[];
  forcedAmbiguity?: SelectorGraphAmbiguity;
} {
  if (relation.nodeId) return { ids: new Set([relation.nodeId]) };
...
function addCriterion(candidates: Candidate[], evidence: ObjectGraphEvidence): Candidate[] {
...
function candidateToMatch(candidate: Candidate): SelectorGraphMatch {
  return {
...
function candidateConfidence(candidate: Candidate): number {
  if (!candidate.confidenceParts.length) return 0;
  if (candidate.confidenceParts.includes(1)) return 1;
...
function detectAmbiguity(candidates: Candidate[]): SelectorGraphAmbiguity {
...
function normalizeTextSelector(selector: SelectorGraphTextSelector): Required<Exclude<SelectorGraphTextSelector, string>> {
...
function textMatches(node: ObjectGraphNode, selector: Required<Exclude<SelectorGraphTextSelector, string>>): boolean {
...
function nodeTextContains(node: ObjectGraphNode, text: string): boolean {
  return normalizeText(node.text?.value ?? node.text?.preview ?? "").includes(normalizeText(text));
}

function normalizeBBoxSelector(selector: SelectorGraphBBoxSelector): Required<Exclude<SelectorGraphBBoxSelector, ObjectGraphBBox>> {
...
function bboxMatches(actual: ObjectGraphBBox | undefined, selector: Required<Exclude<SelectorGraphBBoxSelector, ObjectGraphBBox>>): boolean {
  if (!actual) return false;
  const expected: ObjectGraphBBox = [selector.x, selector.y, selector.width, selector.height];
  if (selector.mode === "intersects") return intersects(actual, expected);
  if (selector.mode === "contains") return contains(actual, expected);
...
function isRightOf(source: ObjectGraphBBox | undefined, target: ObjectGraphBBox | undefined): boolean {
  if (!source || !target) return false;
  return source[0] >= target[0] + target[2] && verticalOverlapRatio(source, target) >= 0.25;
}

function horizontalGap(source: ObjectGraphBBox, target: ObjectGraphBBox): number {
  return Math.max(0, source[0] - (target[0] + target[2]));
}

function centerDistance(node: ObjectGraphNode, x: number, y: number): number {
  if (!node.bbox) return Number.MAX_SAFE_INTEGER;
...
function intersects(left: ObjectGraphBBox, right: ObjectGraphBBox): boolean {
