...
import type { CapabilitiesDocument, CapabilityContract, CapabilityFeature, FeatureName, JsonObject, OfficegenConfig } from "./types.js";
...
export const featureContracts: CapabilityContract[] = [
  {
...
export const knownLimitations = featureContracts.flatMap((contract) =>
  contract.limitations.map((limitation) => `${contract.area}: ${limitation}`)
);
