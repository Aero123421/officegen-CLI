...
export const COMMAND_OPTION_SPECS: Record<string, OptionSpec[]> = {
...
const COMMAND_ACCEPTED_EXTRA_OPTION_SPECS: Record<string, OptionSpec[]> = {
...
export function effectiveOptionsFor(commandGroup: string, subcommand?: string): string[] {
  return effectiveOptionSpecsFor(commandGroup, subcommand).map((spec) => spec.flag);
}

export function acceptedOptionsFor(commandGroup: string, subcommand?: string): string[] {
  return acceptedOptionSpecsFor(commandGroup, subcommand).map((spec) => spec.flag);
}

export function optionSyntax(spec: OptionSpec): string {
  return spec.value ? `${spec.flag} ${spec.value}` : spec.flag;
}

function commandKey(commandGroup: string, subcommand?: string): string {
  return subcommand ? `${commandGroup} ${subcommand}` : commandGroup;
}

function option(flag: string, value: string | undefined, description: string): OptionSpec {
  return { flag, value, description };
}

function uniqueOptionSpecs(specs: OptionSpec[]): OptionSpec[] {
