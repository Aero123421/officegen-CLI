...
function artifactRequiredWhenForHelp(commandGroup: string, subcommand?: string): string[] {
  ...
  if (commandGroup === "improve" || commandGroup === "benchmark") return [];
...
function successConditionForHelp(commandGroup: string): string {
  ...
  if (commandGroup === "design") return "capture writes design/capture artifacts; apply mutates only when a supported Office target and --out are supplied.";
  if (commandGroup === "chart") return "chart render returns SVG and writes --out when supplied.";
  if (commandGroup === "diagram") return "diagram render returns SVG and writes --out when supplied.";
  if (commandGroup === "layout") return "layout apply computes constrained boxes; it mutates PPTX only when targetPath and a .pptx --out are supplied.";
...
function formatCapabilitiesForFeature(feature: FeatureKey): Record<string, unknown> | undefined {
  ...
  if (feature === "benchmark") return { corpusFiles: "external-manifest-only", mutatesOffice: false };
  return undefined;
}
...
async function maybeWriteReport(context: RuntimeContext, payload: unknown, sourceCommand: string): Promise<unknown> {
  ...
  if (!reportOut) return limited;
  const reportPath = await validateOutputPath(context, reportOut);
  const safeReport = redactJson(limited as JsonValue, context.config).value;
...
function applyOutputProjection(context: RuntimeContext, payload: unknown): unknown {
...
function outputPathForValidation(context: RuntimeContext, filePath: string): string {
...
function viewPageDimensions(content: string): { width?: number; height?: number } {
...
export async function renderPayload(context: RuntimeContext): Promise<unknown> {
  ...
  const result = await render(sanitizedIr as Parameters<typeof render>[0], withFormatConfig(context, {
    out,
...
function compactSelectPayload(context: RuntimeContext, payload: Record<string, unknown>): Record<string, unknown> {
...
export async function improvePayload(context: RuntimeContext): Promise<unknown> {
...
function critiqueFindings(format: string, summary: Record<string, unknown>, untrusted: Record<string, unknown>, objectMap: unknown[], profile: string): Array<Record<string, unknown>> {
  ...
  if (format === "pptx") {
    ...
    if (slides > 1 && assets === 0) findings.push(qualityFinding("PPTX_ASSETS_NONE", "warning", "Deck has no image/logo assets; branded business decks usually need at least one visual anchor.", "Add logo/image blocks or run scaffold with a business scenario."));
    if (/kpi|dashboard|board|sales|business/i.test(profile) && charts === 0) findings.push(qualityFinding("PPTX_CHARTS_NONE", "warning", "Business/KPI deck has no charts.", "Add chart blocks or use chartData template bindings."));
    if (slides > 0 && textObjects / slides > 18) findings.push(qualityFinding("PPTX_TEXT_DENSITY_HIGH", "warning", "Average text object density is high; slide readability may be poor.", "Split dense slides or run layout repair/fit-text."));
    ...
    if (/dashboard|kpi|business/i.test(profile) && charts === 0) findings.push(qualityFinding("XLSX_DASHBOARD_CHARTS_NONE", "warning", "Workbook profile expects dashboard charts, but no chart objects were detected.", "Add chart sheets or use xlsx chart update workflow."));
    if (tables === 0) findings.push(qualityFinding("XLSX_TABLES_NONE", "warning", "Workbook has no Excel table objects.", "Use xlsx.writeTable/updateTableRows for structured data."));
    if (formulas === 0) findings.push(qualityFinding("XLSX_FORMULAS_NONE", "info", "No formulas were detected; confirm this is intended for generated workbooks.", "Inspect with --sheet/--range and add formulas where calculations are expected."));
    if (cells > 250) findings.push(qualityFinding("XLSX_WIDE_SHEET_READABILITY_RISK", "warning", "Workbook has many visible cells and may need dashboard summaries.", "Inspect key ranges and add tables/charts for scanability."));
  } else if (format === "docx") {
    ...
    if (duplicateHeading) findings.push(qualityFinding("DOCX_REPEATED_HEADINGS", "warning", "Adjacent repeated headings were detected.", "Collapse duplicate headings or adjust generated outline."));
    if (Number(summary.headers ?? 0) === 0 && /proposal|report|business/i.test(profile)) findings.push(qualityFinding("DOCX_HEADERS_NONE", "info", "Business DOCX has no header parts.", "Add header/footer if the deliverable needs a formal template."));
    const paragraphs = Number(summary.paragraphs ?? objectMap.filter((entry) => asRecord(entry).kind === "paragraph").length);
    if (paragraphs === 0) findings.push(qualityFinding("DOCX_EMPTY_SECTION", "warning", "Document has no detected paragraph content.", "Inspect the DOCX structure and add body sections."));
    const headingLevels = headings.map((heading) => Number(asRecord(heading).level ?? 0)).filter((level) => Number.isFinite(level) && level > 0);
    if (headingLevels.some((level, index) => index > 0 && level - headingLevels[index - 1] > 1)) findings.push(qualityFinding("DOCX_HEADING_HIERARCHY_GAP", "warning", "Heading levels appear to skip hierarchy levels.", "Normalize heading levels before final delivery."));
    const wideTable = objectMap.map(asRecord).some((entry) => entry.kind === "tableCell" && Number(asRecord(entry.bounds).width ?? 0) > 0.9);
    if (wideTable) findings.push(qualityFinding("DOCX_TABLE_WIDTH_RISK", "warning", "A DOCX table cell appears wider than the page-safe content area.", "Review table widths and split wide tables if needed."));
  }
  return findings;
}

function qualityFinding(code: string, severity: string, message: string, repair: string): Record<string, unknown> {
  return {
    code,
    severity,
    category: "quality",
...
function severityPenalty(severity: string): number {
  if (severity === "error") return 0.35;
  if (severity === "warning") return 0.12;
  return 0.03;
}
...
export async function benchmarkPayload(context: RuntimeContext, subcommand?: string): Promise<unknown> {
  if (subcommand === "compare") return benchmarkComparePayload(context);
  ...
  for (const [index, item] of entries.map(asRecord).entries()) {
    ...
    try {
      ...
      const inspected = await inspect(filePath, withFormatConfig(context, { depth: "summary" as const, structure: true }));
...
function benchmarkManifestPath(context: RuntimeContext, subcommand?: string): string {
...
function benchmarkRecoveryCommands(manifestPath: string): string[] {
  return [
    "npm run benchmark:fetch",
...
function resolveBenchmarkStorageRoot(context: RuntimeContext, storageRoot: unknown): string {
  return resolveBenchmarkRelativePath(context, context.cwd, String(storageRoot ?? DEFAULT_BENCHMARK_STORAGE_ROOT), "storageRoot");
}

function resolveBenchmarkDocumentPath(context: RuntimeContext, storageRoot: string, relativePath: string, field: string): string {
  return resolveBenchmarkRelativePath(context, storageRoot, relativePath, field);
}

function assertBenchmarkManifestPathAllowed(context: RuntimeContext, value: string, field: string): void {
  const reason = benchmarkPathDenyReason(value);
  if (reason) throw benchmarkPathDenied(context, value, field, reason);
}

function resolveBenchmarkRelativePath(context: RuntimeContext, root: string, value: string, field: string): string {
  const reason = benchmarkPathDenyReason(value);
  if (reason) throw benchmarkPathDenied(context, value, field, reason);
...
function benchmarkPathDenyReason(value: string): string | undefined {
  if (!value || value.includes("\0")) return "empty or invalid path";
  if (path.isAbsolute(value) || path.win32.isAbsolute(value) || path.posix.isAbsolute(value) || /^[A-Za-z]:/.test(value)) return "absolute path";
...
function benchmarkPathDenied(context: RuntimeContext, requestedPath: string, field: string, reason: string): CliFailure {
  return new CliFailure({
...
async function critiqueResultForBenchmark(context: RuntimeContext, filePath: string): Promise<Record<string, unknown>> {
  const inspected = await inspect(filePath, withFormatConfig(context, { depth: "summary" as const, structure: true }));
  const trusted = asRecord(inspected.trusted);
...
async function benchmarkComparePayload(context: RuntimeContext): Promise<unknown> {
  const args = positionalArgs(context.argv, 4);
...
export async function runPayload(context: RuntimeContext): Promise<unknown> {
  ...
  if (manifestOut) await fs.mkdir(path.dirname(manifestOut), { recursive: true });
  if (summaryOut) await fs.mkdir(path.dirname(summaryOut), { recursive: true });
  if (outputRoot) await fs.mkdir(outputRoot, { recursive: true });
  const expectedArtifacts = await readExpectedArtifacts(context);
  const beforeOutputRoot = outputRoot ? await snapshotFiles(outputRoot) : new Set<string>();
  ...
  for (const [index, step] of steps.entries()) {
    ...
    try {
      ...
      const result = await withTimeout(
        executeRunStep(context, folder, step, stepOutputs, index, outputRoot, denyOutsideOutputRoot, stepTimeoutMs),
        stepTimeoutMs,
        `run step ${id} (${command})`
      );
      ...
      await appendRunJsonl(logJsonl, { event: "step.end", id, command, ok: false, exitCode: 1, envelopeOk: false, resultPath, error: failure.error as JsonValue, finishedAt });
      await appendTrace(folder, { event: "step.end", id, command, ok: false, resultPath, error: failure.error as JsonValue, finishedAt });
      failed = true;
      break;
    }
  }
  const missingExpected = await missingExpectedArtifacts(expectedArtifacts);
  const afterOutputRoot = outputRoot ? await snapshotFiles(outputRoot) : new Set<string>();
  ...
  let finalError: Record<string, unknown> | undefined;
...
async function officeAgentPayload(context: RuntimeContext): Promise<unknown> {
  ...
  if (reportOut) return writeReportPayload(context, result, reportOut, "run office-agent");
  return applyOutputProjection(context, result);
}

async function writeReportPayload(context: RuntimeContext, payload: unknown, reportPath: string, sourceCommand: string): Promise<unknown> {
  const limited = applyOutputProjection(context, payload);
  const safeReport = redactJson(limited as JsonValue, context.config).value;
  await fs.mkdir(path.dirname(reportPath), { recursive: true });
  await fs.writeFile(reportPath, `${JSON.stringify(safeReport, null, 2)}\n`, "utf8");
...
async function prepareReferencePayload(context: RuntimeContext): Promise<unknown> {
  ...
  const [referenceInspect, targetInspect] = await Promise.all([
    inspect(referencePath, withFormatConfig(context, { depth: "full" as const })),
...
function totalPageLikeCount(inspected: Awaited<ReturnType<typeof inspect>>): number {
...
async function assertRunStepOutcome(command: string, result: unknown, artifacts: Array<Record<string, unknown>>, step: Record<string, unknown>): Promise<void> {
  const record = asRecord(result);
  ...
  if ((command === "edit" || command === "repair") && !dryRun) {
    ...
    if (outputArtifact) await assertValidOoxmlMutationOutput(String(outputArtifact.path), `run ${command}`);
...
function runEditFailureCode(errors: unknown[]): "SELECTOR_NOT_FOUND" | "SELECTOR_AMBIGUOUS" | "EDIT_TRANSACTION_FAILED" {
  const text = errors.map((error) => JSON.stringify(error)).join("\n").toLowerCase();
  if (text.includes("ambiguous")) return "SELECTOR_AMBIGUOUS";
  if (text.includes("not-found") || text.includes("not found") || text.includes("missing")) return "SELECTOR_NOT_FOUND";
...
async function appendRunJsonl(filePath: string | undefined, record: Record<string, unknown>): Promise<void> {
  if (!filePath) return;
...
async function readExpectedArtifacts(context: RuntimeContext): Promise<Array<Record<string, unknown>>> {
  ...
  const items = Array.isArray(raw) ? raw : Array.isArray(asRecord(raw).artifacts) ? asRecord(raw).artifacts as unknown[] : [];
  return items.map((item) => {
    if (typeof item === "string") return { path: path.resolve(context.cwd, item), expected: true };
    const record = asRecord(item);
    return { ...record, path: path.resolve(context.cwd, String(record.path ?? "")), expected: true };
  });
}

async function missingExpectedArtifacts(expected: Array<Record<string, unknown>>): Promise<Array<Record<string, unknown>>> {
  const missing = [];
  for (const artifact of expected) {
    ...
    if (!record.exists) missing.push({ ...artifact, ...record, reason: "expected artifact was not created" });
  }
  return missing;
}

async function collectResultArtifacts(result: unknown, command: string, input?: string): Promise<Array<Record<string, unknown>>> {
  const record = asRecord(result);
  const artifacts = [];
...
async function assertSafeOoxmlMutationInput(inputPath: string, command: string): Promise<void> {
  const extension = path.extname(inputPath).toLowerCase().slice(1);
  if (extension !== "pptx" && extension !== "docx" && extension !== "xlsx") return;
  const riskyParts = await detectOoxmlRiskyParts(inputPath, { format: extension });
  if (!riskyParts.length) return;
  throw new CliFailure({
...
async function assertValidOoxmlMutationOutput(outputPath: string, command: string): Promise<void> {
...
async function artifactRecord(filePath: string, kind: string, format: string, createdByCommand: string, input?: string): Promise<Record<string, unknown>> {
  try {
...
async function snapshotFiles(root: string): Promise<Set<string>> {
...
function isOutside(root: string, filePath: string): boolean {
  const relative = path.relative(path.resolve(root), path.resolve(filePath));
  return relative.startsWith("..") || path.isAbsolute(relative);
}

function runSummaryMarkdown(manifest: Record<string, any>): string {
  const lines = [
    ...
    "",
    "## Steps",
    ...(manifest.steps ?? []).map((step: Record<string, unknown>) => `- ${step.ok ? "ok" : "failed"} ${step.id}: ${step.command}`)
  ];
  return `${lines.join("\n")}\n`;
}
...
function runStepFailurePayload(id: string, command: string, error: unknown): Record<string, unknown> {
...
export async function assetPayload(context: RuntimeContext, subcommand?: string): Promise<unknown> {
  ...
  if (subcommand === "inspect" || !subcommand) {
    const inputPath = await validateInputPath(context, input);
    if (hasFlag(context.argv, "--embedded")) return inspectEmbeddedAssets(inputPath, withFormatConfig(context, {}));
...
async function templateCandidateArtifacts(candidates: unknown[]): Promise<Array<Record<string, unknown>>> {
  const files = new Set<string>();
...
function summarizeTemplateCandidate(candidate: unknown): Record<string, unknown> {
...
function summarizeTemplateField(field: unknown): Record<string, unknown> {
...
function summarizeTemplateMapCandidate(candidate: unknown): Record<string, unknown> {
  const record = asRecord(candidate);
  return {
    field: record.field,
...
function summarizeTemplateSchemaCandidate(candidate: unknown): Record<string, unknown> {
  const record = asRecord(candidate);
  return {
    name: record.name,
    type: record.type,
...
function summarizeTemplateMapSuggestion(value: unknown): Record<string, unknown> | undefined {
...
function summarizeArray(value: unknown, summarize: (item: unknown) => Record<string, unknown>, limit = 25): Record<string, unknown> {
...
async function designCaptureArtifacts(optional: ReturnType<typeof optionalContext>, id: string, captured: unknown): Promise<Array<Record<string, unknown>>> {
